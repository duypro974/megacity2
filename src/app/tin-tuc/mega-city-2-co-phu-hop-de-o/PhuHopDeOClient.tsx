...
// ── An cư data: tiện ích ngoại khu ─────────────────
const nearbyFacilities = [
  {
    category: "Giáo dục",
    items: [
      { name: "Trường THCS Phú Hội", distance: "~2 km", note: "Trong xã Phú Hội" },
      { name: "Trường Tiểu học Phú Hội", distance: "~2 km", note: "Trong xã Phú Hội" },
      { name: "Trường THPT Nhơn Trạch 3", distance: "~4 km", note: "Khu vực Nhơn Trạch" },
      { name: "Các trường tư thục Biên Hòa", distance: "~25 km", note: "Theo tuyến đường 319" },
    ],
  },
  {
    category: "Y tế",
    items: [
      { name: "Trạm y tế xã Phú Hội", distance: "~1,5 km", note: "Khám cơ bản, tiêm ngừa" },
      { name: "Bệnh viện Nhơn Trạch", distance: "~8 km", note: "Tuyến huyện" },
      { name: "BV Đồng Nai (Biên Hòa)", distance: "~25 km", note: "Tuyến tỉnh, ~35 phút" },
      { name: "BV Quận 2 / Thủ Đức (TP.HCM)", distance: "~20 km qua phà", note: "Phà Cát Lái ~30 phút" },
    ],
  },
  {
    category: "Mua sắm & Dịch vụ",
    items: [
      { name: "Chợ Phú Hội", distance: "~2 km", note: "Chợ dân sinh cơ bản" },
      { name: "Siêu thị khu vực Nhơn Trạch", distance: "~6–8 km", note: "Theo tuyến đường 25C" },
      { name: "AEON / Vincom Biên Hòa", distance: "~28 km", note: "~40 phút theo đường 25C" },
    ],
  },
];

// ── An cư data: thời gian di chuyển ─────────────────
const commuteRows = [
  { destination: "Quận 1 – Trung tâm TP.HCM", route: "Đường 25C → Cao tốc Bến Lức–Long Thành", offPeak: "45–55 phút", peak: "65–90 phút" },
  { destination: "Quận 2 / Thủ Đức (TP.HCM)", route: "Đường 25C → Phà Cát Lái", offPeak: "30–40 phút", peak: "50–70 phút" },
  { destination: "TP. Biên Hòa", route: "Đường 25C → Đường 319", offPeak: "30–40 phút", peak: "45–60 phút" },
  { destination: "Long Thành", route: "Đường 25C", offPeak: "15–20 phút", peak: "20–30 phút" },
  { destination: "KCN Nhơn Trạch 1–3", route: "Đường nội bộ Nhơn Trạch", offPeak: "8–15 phút", peak: "10–20 phút" },
  { destination: "Sân bay Long Thành (đang xây)", route: "Đường 25C → nhánh vào sân bay", offPeak: "~20–25 phút*", peak: "~25–35 phút*" },
];

// ── An cư data: chi phí tổng hợp ─────────────────
const totalCostRows = [
  { item: "Giá đất (nhà phố T nội khu)", cost: "1,3 – 2,2 tỷ", note: "Giá tham khảo 2026" },
  { item: "Chi phí xây nhà (3 tầng ~90m²)", cost: "1,5 – 2,5 tỷ", note: "Tùy vật liệu & thiết kế" },
  { item: "Nội thất cơ bản", cost: "200 – 500 triệu", note: "Tùy mức độ hoàn thiện" },
  { item: "Thuế trước bạ & phí công chứng", cost: "~15–25 triệu", note: "~0,5–1% giá trị đất" },
  { item: "Lãi vay (nếu vay 50% giá đất)", cost: "~4–7 triệu/tháng", note: "Lãi suất 8–10%/năm, kỳ hạn 15–20 năm" },
  { item: "Chi phí sinh hoạt ước tính/tháng", cost: "~15–25 triệu/tháng", note: "Gia đình 4 người tại Nhơn Trạch" },
];
...
// Page Component
// ─────────────────────────────────────────────────────────────
export default function PhuHopDeOClient() {
          ...
          <div className="flex flex-col lg:flex-row gap-16">
            ...
            <article className="flex-1 min-w-0">
              ...
              <section className="mb-14">
                ...
                <div className="pt-6 space-y-5">
                  <p className="text-slate-600 text-[17px] leading-[1.85]">
                    Đây là một trong những nhóm khách hàng nên xem xét kỹ nhất trước khi quyết định. Ngoài việc nhìn vào quy hoạch trên giấy, cần đánh giá tiện ích thực tế đang hoạt động trong bán kính di chuyển hàng ngày.
                  </p>

                  {/* Tiện ích ngoại khu bảng */}
                  <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="bg-slate-900 px-6 py-4">
                      <p className="text-white font-black text-sm uppercase tracking-wider">Tiện ích dân sinh khu vực Nhơn Trạch – ước tính khoảng cách từ Mega City 2</p>
                    </div>
                    {nearbyFacilities.map((cat) => (
                      <div key={cat.category}>
                        <div className="bg-amber-50 border-y border-amber-100 px-5 py-2">
                          <p className="text-xs font-black text-amber-800 uppercase tracking-wider">{cat.category}</p>
                        </div>
                        <div className="divide-y divide-slate-100">
                          {cat.items.map((item) => (
                            <div key={item.name} className="grid grid-cols-3 px-5 py-3 text-sm hover:bg-slate-50 transition-colors">
                              <span className="font-semibold text-slate-700 text-xs leading-snug">{item.name}</span>
                              <span className="font-black text-amber-600 text-xs text-center">{item.distance}</span>
                              <span className="text-slate-400 text-xs text-right">{item.note}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="px-5 py-3 bg-slate-50 border-t border-slate-100">
                      <p className="text-[11px] text-slate-400 italic">* Khoảng cách ước tính, có thể thay đổi theo tuyến đường thực tế. Nên xác nhận lại trước khi quyết định.</p>
                    </div>
                  </div>

                  <InfoBox type="warn">
                    <strong>Thực tế cần kiểm tra:</strong> Nhơn Trạch đang trong giai đoạn phát triển — hệ thống trường học và y tế tuyến huyện/xã còn hạn chế so với khu vực đô thị đã phát triển. Gia đình có con nhỏ cần đến học tại trường tư thục chất lượng cao hoặc bệnh viện tuyến tỉnh sẽ cần tính thêm thời gian di chuyển 25–40 phút mỗi chiều.
                  </InfoBox>
                </div>
              </section>
              ...
              <section className="mb-14">
                ...
                <div className="pt-6 space-y-5">
                  ...
                  <p className="text-slate-600 text-[17px] leading-[1.85]">
                    Tuy nhiên, nếu bạn làm việc tại TP.HCM và phải di chuyển mỗi ngày, hãy tính thời gian cụ thể theo tuyến đường và giờ di chuyển thực tế — không chỉ nhìn vào khoảng cách km.
                  </p>

                  {/* Bảng thời gian di chuyển */}
                  <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="bg-slate-900 px-6 py-4">
                      <p className="text-white font-black text-sm uppercase tracking-wider">Thời gian di chuyển từ Mega City 2 (ước tính 2026)</p>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase">Điểm đến</th>
                            <th className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase hidden sm:table-cell">Tuyến đường</th>
                            <th className="text-center px-4 py-3 text-xs font-bold text-emerald-600 uppercase">Ngoài giờ cao điểm</th>
                            <th className="text-center px-4 py-3 text-xs font-bold text-amber-600 uppercase">Giờ cao điểm</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {commuteRows.map((row, i) => (
                            <tr key={row.destination} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                              <td className="px-5 py-3.5 font-semibold text-slate-700 text-xs">{row.destination}</td>
                              <td className="px-4 py-3.5 text-slate-500 text-xs hidden sm:table-cell">{row.route}</td>
                              <td className="px-4 py-3.5 text-center font-black text-emerald-600 text-xs">{row.offPeak}</td>
                              <td className="px-4 py-3.5 text-center font-black text-amber-600 text-xs">{row.peak}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="px-5 py-3 bg-slate-50 border-t border-slate-100">
                      <p className="text-[11px] text-slate-400 italic">* Ước tính theo điều kiện giao thông 2026. Thực tế có thể thay đổi theo tuyến đường và phương tiện. Sân bay Long Thành đang xây dựng, thời gian di chuyển tham khảo.</p>
                    </div>
                  </div>

                  <InfoBox>
                    Nếu gia đình làm việc chủ yếu tại Nhơn Trạch, Long Thành hoặc các KCN lân cận (8–20 phút), thời gian di chuyển là lợi thế rõ rệt. Nếu làm việc tại trung tâm TP.HCM Q1–Q3, nên thử tuyến đường thực tế vào giờ cao điểm (6h45–8h sáng) trước khi quyết định.
                  </InfoBox>
              ...
              <section className="mb-14">
                ...
                <div className="pt-6 space-y-5">
                  ...
                  {/* Bảng chi phí an cư tổng hợp */}
                  <p className="text-slate-600 text-[17px] leading-[1.85]">
                    Dưới đây là ước tính chi phí tổng hợp để an cư tại Mega City 2 — để gia đình có con số thực tế cần chuẩn bị:
                  </p>
                  <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="bg-slate-900 px-6 py-4">
                      <p className="text-white font-black text-sm uppercase tracking-wider">Chi phí an cư tổng hợp tại Mega City 2 (ước tính 2026)</p>
                    </div>
                    <div className="divide-y divide-slate-100">
                      {totalCostRows.map((row, i) => (
                        <div key={row.item} className={`grid grid-cols-3 px-5 py-4 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/60"}`}>
                          <span className="font-semibold text-slate-700 text-xs col-span-1 leading-snug">{row.item}</span>
                          <span className="font-black text-amber-600 text-xs text-center">{row.cost}</span>
                          <span className="text-slate-400 text-xs text-right leading-snug">{row.note}</span>
                        </div>
                      ))}
                    </div>
                    <div className="px-5 py-3 bg-amber-50 border-t border-amber-100">
                      <p className="text-[11px] text-amber-800 font-semibold">Tổng vốn tự có tối thiểu nên chuẩn bị: <span className="text-amber-700">~2–3 tỷ đồng</span> (50% giá đất + xây nhà cơ bản) để hệ số vay không quá căng.</p>
                    </div>
                    <div className="px-5 py-2.5 bg-slate-50 border-t border-slate-100">
                      <p className="text-[11px] text-slate-400 italic">* Chi phí ước tính, không bao gồm biến động thị trường. Liên hệ tư vấn cụ thể theo từng lô.</p>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <div className="mb-14 rounded-2xl border-2 border-slate-200 bg-slate-50 p-6">
                ...
                <p className="font-bold text-slate-800 text-base mb-2">Bạn đang cân nhắc Mega City 2 theo góc độ đầu tư?</p>
                <p className="text-slate-500 text-sm leading-relaxed mb-4">
                  Phân tích đầu tư có các tiêu chí khác: lợi suất cho thuê, thanh khoản, dòng tiền kỳ vọng và so sánh thị trường khu vực — xem bài riêng để có góc nhìn đầy đủ hơn.
                </p>
                <a href="/tin-tuc/co-nen-dau-tu-mega-city-2-nhon-trach"
                  ...
                  Có nên đầu tư Mega City 2 năm 2026? →
                </a>
              </div>
